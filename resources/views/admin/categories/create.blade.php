...
@section('content')
    <div class="max-w-2xl bg-white rounded-3xl border border-neutral-200 shadow-card p-6">
        <form action="{{ route('admin.kategori.store') }}" method="POST" class="space-y-4" data-validate>
            @csrf

            <div>
                <label for="name" class="block text-sm font-semibold text-neutral-700">Nama Kategori</label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" required maxlength="100" data-required data-label="Nama kategori" data-max="100"
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="rounded-full bg-ink px-6 py-2.5 text-sm font-bold text-white shadow-card hover:shadow-hover transition">Simpan</button>
