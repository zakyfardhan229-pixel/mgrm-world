    ...
    <div class="bg-white rounded-3xl border border-neutral-200 shadow-card p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <form action="{{ route('admin.kategori.index') }}" method="GET"
                ...
                <button type="submit"
                    class="rounded-full bg-ink px-4 py-2 text-sm font-bold text-white shadow-card hover:shadow-hover transition">Cari</button>
                @if (request('search'))
                    <a href="{{ route('admin.kategori.index') }}"
                        class="text-sm font-semibold text-neutral-500 hover:text-neutral-900">Reset</a>
                @endif
            </form>
        ...
        @if ($categories->isEmpty())
            <div class="mt-8 text-center py-10">
                <p class="font-semibold text-neutral-900">Tidak ada kategori ditemukan</p>
            </div>
        @else
                ...
                <table class="w-full min-w-[640px] text-sm">
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @foreach ($categories as $category)
                            <tr>
                                <td class="py-3.5 pr-4 font-bold text-neutral-900">{{ $category->name }}</td>
                                <td class="py-3.5 pr-4 text-neutral-500">{{ $category->slug }}</td>
                                <td class="py-3.5 pr-4 text-neutral-500 max-w-xs truncate">{{ $category->description ?: '—' }}</td>
                                <td class="py-3.5 pr-4"><span
                                        class="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-bold text-neutral-600">{{ $category->products_count }}</span>
                                </td>
                                <td class="py-3.5">
